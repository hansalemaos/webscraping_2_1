...
df.loc[df.aa_text == "Login"].iloc[0].ff_input_tap()
adb.is_keyboard_shown()
adb.input_text("baba@bibi.com", remove_accents=True, sleep_after_letter=(0, 0.5))
adb.keyevents.KEYCODE_TAB.press()
adb.input_text("senha", remove_accents=True, sleep_after_letter=(0, 0.5))
df = adb.plus_uidump_with_freeze(
    procregex_for_lsof=rb"^\s*dboxed_process.*kiwi",
    timeout=15,
...
df.plus_save_screenshots(r"C:\betshots2")
df.loc[df.aa_text.str.contains("Ficar")].iloc[0].ff_input_tap(-55, 0)